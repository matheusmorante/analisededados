...
class DataAnalysisApp:
    ...
    def createWidgets(self):
        ...
        self.inventoryButton = tk.Button(self.root, text="Selecione o arquivo CSV de estoque", command=self.selectInventory_file)
        self.inventoryButton.grid(row=1, column=0)

        self.leadtimeButton = tk.Button(self.root, text="Selecione o arquivo CSV de Lead time de fornecedores", command=self.select_leadtime_file)
    ...
    def startAnalysis(self):
        ...
        try:
            os.system(f"hdfs dfs -put {self.sales_file} /data/sales.csv")
            os.system(f"hdfs dfs -put {self.inventory_file} /data/inventory.csv")
            os.system(f"hdfs dfs -put {self.leadtime_file} /data/suppliers_leadtime.csv")
